...
class FormAdmin(admin.ModelAdmin):
    ...
    def queryset(self, request):
        ...
        #if user is super user show all
        if request.user.is_superuser:
            return qs.annotate(total_entries=Count("entries"))
        qs = qs.annotate(total_entries=Count("entries"))
        groups = request.user.group.all()
        group = groups[0]
        return qs.filter(created_by=group)
